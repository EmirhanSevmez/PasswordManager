...
def move_to_secret():
    if not os.path.isdir(".passwords"): # Checking if the directory exists
        os.mkdir(".passwords") # Creating the directory
    os.chdir(".passwords") # Changing the directory to .passwords
...
def encrypt(): # Function to encrypt the password
    ...
    with open("secrets", "rb") as file: # Opening the file in read binary mode
        data = file.read() # Reading the data from the file
    encrypted_data = cipher_suite.encrypt(data) # Encrypting the data
    with open("secrets", "wb") as file: # Opening the file in write binary mode
...
def decrypt(): # Function to decrypt the password
    ...
    with open("secrets", "w") as file: # Opening the file in write mode
...
def Add(): # Function to add a new password
    ...
    if not username.strip() or not password.strip(): # Checking if the username or password is empty
        ...
        with open("secrets", "a") as f: # Opening the file in append mode
...
def delete(): # Function to delete a password
    ...
    if not username.strip(): # Checking if the username is empty
            ...
            return # Returning from the function
    with open("secrets", "r") as f: # Opening the file in read mode
        lines = f.readlines() # Reading all the lines from the file
    with open("secrets", "w") as f: # Opening the file in write mode
...
move_to_secret() # Moving the file to the appdata folder
...
try:
    with open("secrets", "rb") as password_file: # Opening the password file in read binary mode
        password_file.read() # Reading the password file
    decrypt()
except FileNotFoundError: # If the password file is not found
    with open("secrets", "w") as password_file: # Opening the password file in write mode
    ...
    try: 
        ...
        if i == 1: # If the user selects option 1
            ...
            if x == 1: # If the user selects option 1
                ...
                print("Invalid option") # Print an error message
